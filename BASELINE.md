...
- every authoritative runtime trace event now routes through one singular durable emission authority (`_record_trace_event`); the orchestrator no longer contains direct `self.trace.add_event(...)` calls outside that boundary (AST-guarded); operator trace events carry first-class `trace_event_id` and appear in the authoritative trace projection;
...
4. ~~Route every accepted runtime trace event through one durable emission authority.~~ **Closed in H0.4 (implementation pin `b89c9dd7ba2890d4fa66f89b2b682f036446a591`).** `_record_trace_event` is the singular authority; durable-first, one shared identity; AST guard enforces exactly one `.add_event()` call in all of `src/nodechain/`.
