...
## 6d. Human-readable final research artifact (H1.4)

Any verified terminal bundle can be turned into a readable research memo through `nodechain research report <run-id> [--workspace DIR] [--output PATH] [--json]`. The memo is a deterministic view of evidence the governed run already produced: no model call, no network, no node execution, no state mutation. The command is runtime-state read-only; its only side effect is the explicit `--output` Markdown artifact (written as bytes, so line endings are stable across platforms). Absent or invalid bundles fail nonzero and create no artifact; existing output paths are never silently overwritten.

Before rendering, the terminal projection was completed to carry the evidence it already claimed: `report.json` preserves the final Response Generator's recommendation, executive summary, key findings, confidence statement, alternatives, and methodology plus the risk classifier's level/confidence/factors/review reason (all optional, backward-compatible schema fields); `uncertainties.json` projects actual risk disclosures (claim attribution is never inferred); `review-decisions.json` carries ADMITTED runtime review decisions only — `review_completed` requires completion evidence, never merely the negation of `review_required`; `failures.json` normalizes immutable fault records into the published failure-record contract (dispatch occurrence and evidence availability from the record; claim impact only when established); and citation/evidence/claim links are completed from existing source/evidence relationships. The sealed fixture model adapter emits the Response Generator's declared schema so fixture runs preserve the same final-response shape as live runs.

`ResearchMemoV1` (`nodechain.research.report`) is the frozen presentation model, constructed only from a `BundleReader`-verified bundle and carrying explicit unavailable-field semantics: pre-H1.4 bundles render with "not available in this legacy bundle" statements, which are never equivalent to "no uncertainty was recorded in this verified bundle." Rendering is deterministic — the same verified bundle produces byte-identical Markdown and structurally identical JSON, with no timestamps, random IDs, or environment-specific paths. The memo's governance appendix (bundle digest, terminal status, adapters actually used, validation count, trace event count/reference) matches the canonical documents. A live run's memo states artifact-bounded reproducibility and never claims deterministic replay. The `.md` artifact lives outside the canonical bundle (`BUNDLE_FILES` unchanged; no Bundle V2) and identifies the source bundle digest inside the document.
