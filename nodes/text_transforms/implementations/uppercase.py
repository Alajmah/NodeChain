"""Uppercase Node implementation."""

from __future__ import annotations
from typing import Any

from nodechain.core.envelope import InvocationEnvelope, EnvelopeResponse
from nodechain.core.contract import NodeContract, EntryContract, ExitContract, Requirements
from nodechain.core.manifest import NodeManifest
from nodechain.core.port import PortType
from nodechain.nodes.base_node import BaseNode

UPPERCASE_CONTRACT = NodeContract(
    contract_id="transforms.uppercase.v1",
    node_id="uppercase_node",
    version="1.0.0",
    entry=EntryContract(
        input_type=PortType.RAW_QUERY,
        schema_ref="nodechain://schemas/semantic_types/raw_user_query",
        required_fields=["query"],
    ),
    exit=ExitContract(
        output_type=PortType.RAW_QUERY,
        schema_ref="nodechain://schemas/semantic_types/raw_user_query",
        guaranteed_fields=["query", "transformed"],
    ),
    requirements=Requirements(model_required=False),
)


class UppercaseNode(BaseNode):
    """Converts input to uppercase."""

    def __init__(self, **kwargs: Any) -> None:
        pass

    @property
    def manifest(self) -> NodeManifest:
        return NodeManifest(
            node_id="uppercase_node", node_type="deterministic",
            name="Uppercase Node",
            description="Converts input query to uppercase.",
            contract=UPPERCASE_CONTRACT,
        )

    async def execute(self, envelope: InvocationEnvelope) -> EnvelopeResponse:
        query = envelope.payload.get("query", "")
        return EnvelopeResponse(
            request_envelope_id=envelope.envelope_id,
            run_id=envelope.run_id, chain_id=envelope.chain_id,
            node_id="uppercase_node", step_id=envelope.step_id,
            output={"query": query, "transformed": query.upper()},
            output_type=PortType.RAW_QUERY,
            cost_usd=0.0, latency_ms=0,
        )
